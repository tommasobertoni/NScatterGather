...
namespace NScatterGather.Recipients
{
    public class RecipientTests
    {
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new Recipient((null as object)!));
        }

        [Fact]
        public void Error_if_type_is_null()
        {
            Assert.Throws<ArgumentNullException>(() => new Recipient((null as Type)!));
        }

        [Fact]
        public void Error_if_request_type_is_null()
        {
            var recipient = new Recipient(typeof(SomeType));
        ...
        [Fact]
        public void Recipient_accepts_input_parameter_generic_type()
        {
            var recipient = new Recipient(typeof(SomeType));
            bool canAccept = recipient.CanAccept<int>();
            Assert.True(canAccept);
        }
        ...
        {
            Assert.Throws<InvalidOperationException>(() => new Recipient(typeof(SomeTypeWithConstructor)));
        }

        [Fact]
        public void Recipient_can_be_created_from_type()
        {
            new Recipient(typeof(SomeType));
        }

        [Fact]
        public void Recipient_can_be_created_from_instance()
        {
            new Recipient(new SomeType());
        }
        ...
        public void Recipient_type_is_visible()
        {
            ...
            var recipient = new Recipient(type);
        ...
        [Fact]
        public void Recipient_replies_with_generic_type()
        {
            var recipient = new Recipient(typeof(SomeType));
            bool canAccept = recipient.CanReplyWith<int, string>();
            Assert.True(canAccept);
        }
        ...
        {
            var recipient = new Recipient(typeof(SomeAsyncType));
            var input = 42;
            var response = await recipient.Accept(input);
            Assert.Equal(input.ToString(), response);
        }
        ...
        {
            var recipient = new Recipient(typeof(SomeComputingType));
            bool accepts = recipient.CanAccept<int>();
        ...
        {
            var recipient = new Recipient(typeof(SomeAsyncComputingType));
